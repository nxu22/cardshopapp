...
class AddDeviseToAdminUsers < ActiveRecord::Migration[7.1]
  def self.up
    change_table :admin_users do |t|
      ...
      ## Lockable
      # t.integer  :failed_attempts, default: 0, null: false unless column_exists?(:admin_users, :failed_attempts) # Only if lock strategy is :failed_attempts
      # t.string   :unlock_token unless column_exists?(:admin_users, :unlock_token) # Only if unlock strategy is :email or :both
